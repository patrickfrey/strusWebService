...
WebRequestLogger::WebRequestLogger( const std::string& logfilename_, int verbosity, int level_, int structDepth_, int maxnofthreads, int procid_, int nofprocs_)
	...
	,m_level((WebRequestLoggerInterface::Level)level_)
	,m_gotAlert(false)
	,m_procid(procid_)
...
void WebRequestLogger::print( const Level level, const char* tag, const char* msg, size_t msglen) noexcept
{
	if (level >= m_level)
	{
		enum {MsgBufSize=2048};
		if (msglen < MsgBufSize || msg[msglen])
		{
			char buf[ MsgBufSize];
			size_t nn = msglen >= sizeof(buf) ? (sizeof(buf)-1):msglen;
			std::memcpy( buf, msg, nn);
			buf[ nn] = 0;
			logMessage( "%s %s %s", levelName( level), tag, buf);
		}
		else
		{
			logMessage( "%s %s %s", levelName( level), tag, msg);
		}
	}
